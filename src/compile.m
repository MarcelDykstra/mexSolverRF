if strcmpi(computer('arch'), 'win32') || strcmpi(computer('arch'), 'win64')
    ...
    mex mex_rf.cu -largeArrayDims -outdir ..
    cd ..
    warn = warning('off', 'all');
    loadlibrary('mex_rf','src/mex_rf.h', 'mfilename', 'mex_rf_proto')
    warning(warn);
    cd src
elseif strcmpi(computer('arch'), 'glnxa64')
    setenv('MW_NVCC_PATH','/opt/cuda/bin')
    mex mex_rf.cu -largeArrayDims -outdir ../ ...
        -I/opt/cuda/include -I. ...
        -L/opt/cuda/lib64 -lcusolver -lcublas -lcusparse -lcudart -ldl
    cd ..
    warn = warning('off', 'all');
    loadlibrary('mex_rf','src/mex_rf.h', 'mfilename', 'mex_rf_proto')
    warning(warn);
